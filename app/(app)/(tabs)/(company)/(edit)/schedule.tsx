...
import { DataDays, DayName, Period } from '@/types/Schedule';

import useGlobal from 'core/globals';
import { Colors } from 'lib';

import Entypo from "@expo/vector-icons/Entypo";
import { createTimeString, parseTimeString, validateTimeRange } from 'utils/TimeUtiles';
import { updateCompany } from 'services/api/company.api';
import LoadingScreen from 'components/LoadingScreen';
import { ScrollView } from 'react-native-gesture-handler';

import { DayRow, TimePickerModal } from 'components/ScheduleEdit';
...
export default function EditSchedule() {
    ...
    const handleSave = useCallback(async () => {
        try {
            setLoading(true);
            await updateCompany({ businessHours: openDays }).then(() => {
                navigation.goBack();
            });
        } catch (error) {
            Alert.alert(
