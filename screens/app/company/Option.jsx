...
const OptionScreen = ({ route }) => {
   ...
   useEffect(() => {
      navigation.setOptions({
         headerRight: () =>
            getValue(companyData, dataOption.key) !== value ? (
               <TouchableOpacity
                  onPress={handleSaveData}
                  disabled={getValue(companyData, dataOption.key) === value}
      ...
      <View className="flex flex-col flex-1">
         {loading ? (
               ...
               <View className="py-1 px-2 rounded-lg flex items-center flex-row border border-dark">
                  <View className="flex flex-col flex-1" style={{ gap: 0 }}>
                     ...
                     <TextInput
                        ...
                        autoCapitalize="none"
                        keyboardType={
                           dataOption?.key === "phone" && "phone-pad"
                        }
                     />
                  </View>
            ...
            <PhoneInput
               ...
               defaultCode={
                  getValue(companyData, dataOption.key)
                     ? CountryCodeMap.find(
                        (c) =>
                           c.dial_code ===
                           getValue(companyData, dataOption.key).substring(
                              0,
                              getValue(companyData, dataOption.key).length -
                              10,
                           ),
                     ).code
                     : "MX"
